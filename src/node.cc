...
#if ENABLE_TTD_NODE
  ab->TTDRawBufferModifyNotifySync(args[0].As<Uint32Array>()->ByteOffset(), 3 * sizeof(uint32_t));
#endif
}
...
#if ENABLE_TTD_NODE
  ab->TTDRawBufferModifyNotifySync(args[0].As<Float64Array>()->ByteOffset(), 2 * sizeof(double));
#endif
}
...
static void StartNodeInstance_TTDReplay(void* arg) {
    ...
    try
    {
        ...
        {
            continueReplayActions = v8::Isolate::RunSingleStepOfReverseMoveLoop(isolate, &s_ttdStartupMode, &nextEventTime);
        }
    }
    catch(...)
    {
